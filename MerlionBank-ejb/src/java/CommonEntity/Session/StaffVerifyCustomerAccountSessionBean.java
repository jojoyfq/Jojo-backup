...
//staff verify customer and choose"reject" or "approve"
@Override
public boolean verifyDiscretionaryAccountCustomer (Long staffID, Long customerID, String result,Long accountId) throws EmailNotSendException{
    Query queryStaff = em.createQuery("SELECT a FROM Staff a WHERE a.id = :id");
        queryStaff.setParameter("id", staffID);
        Staff staff = (Staff)queryStaff.getSingleResult();
    ...
    Query q = em.createQuery("SELECT a FROM Customer a WHERE a.id = :id");
        ...
        Query query = em.createQuery("SELECT a FROM DiscretionaryAccount a WHERE a.id = :id");
        query.setParameter("id", accountId);
        DiscretionaryAccount discretionaryAccount = (DiscretionaryAccount)query.getSingleResult();
        ...
        if (result.equals("approve")){
            customer.setStatus("inactive");
            em.persist(customer);
            StaffAction action=new StaffAction(Calendar.getInstance().getTime(),"Approve discretionary account"+discretionaryAccount.getAccountNumber(),customerID, staff);
            em.persist(action);
            List<StaffAction> staffActions=staff.getStaffActions();
            staffActions.add(action);
            staff.setStaffActions(staffActions);
            ...
            try {
            sendDiscretionaryAccountEmail(customer.getName(),customer.getEmail(),tempPassword,discretionaryAccount.getAccountNumber());
            } catch (MessagingException ex) {
            System.out.println("Error sending email.");
            throw new EmailNotSendException("Error sending email.");
        }
            ...
            customer.setStatus("terminated");
            StaffAction action=new StaffAction(Calendar.getInstance().getTime(),"Reject discretionaryAccount account"+discretionaryAccount.getAccountNumber(),customerID, staff);
            em.persist(action);
            List<StaffAction> staffActions=staff.getStaffActions();
            staffActions.add(action);
            staff.setStaffActions(staffActions);
...
private void sendDiscretionaryAccountEmail(String name, String email, String password,Long accountNumber) throws MessagingException {
        String subject = "Merlion Bank - Online Banking Account \"" + name + "\" Created - Pending Activation";
        System.out.println("Inside send email");
        ...
        String content = "<h2>Dear " + name
                + ",</h2><br /><h1>  Congratulations! You have successfully registered a Merlion Online Banking Account!</h1><br />"
                + "<h1>Welcome to Merlion Bank.</h1>"
                + "<h2 align=\"center\">Discretionary Account Number: " + accountNumber
                + "<br />Temporary Password: " + password + "<br />Please activate your account through activation link on the Login page " + "</h2><br />" 
